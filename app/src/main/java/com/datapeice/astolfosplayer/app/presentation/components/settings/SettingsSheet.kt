...
import androidx.compose.material.icons.filled.Dns // <-- Добавьте этот импорт
                ...
                composable<SettingsRoutes.Main> {
                    ...
                    ColumnWithCollapsibleTopBar(
                        ...
                        val settings = remember {
                            listOf(
                                SettingsItem(
                                    title = context.resources.getString(R.string.server_setting),
                                    supportingText = context.resources.getString(R.string.server_setting_supported_text),
                                    icon = Icons.Filled.Dns,
                                    onClick = {
                                        navController.navigate(SettingsRoutes.Server)
                                    }
                                ),
                                SettingsItem(
...
// В файле SettingsSheet.kt

// ...

                composable<SettingsRoutes.Server> {
                    // Проверяем, что все зависимости не null, перед тем как вызывать экран
                    if (state.settings != null && state.setupViewModel != null) {
                        ServerSettings(
                            settings = state.settings,
                            setupViewModel = state.setupViewModel,
                            onBackClick = {
...
// ...
                ...
                composable<SettingsRoutes.MusicScan> {
                    // Проверяем, что зависимости не null
                    if (state.settings != null && state.musicScanner != null) {
                        MusicScanSettings(
                            settings = state.settings,
                            musicScanner = state.musicScanner,
                            onFolderPick = {
                                onFolderPick(false) // <-- Передаем false, так как это не первоначальная настройка
                            },
                            onBackClick = {
                                navController.navigateUp()
                            },
